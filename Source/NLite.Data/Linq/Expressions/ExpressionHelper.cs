...
namespace NLite.Data.Linq.Expressions
{
    ...
    //            if (keys[i] != null && NLite.Data.Linq.Internal.ReflectionHelper.GetNonNullableType(keys[i].Type) != NLite.Data.Linq.Internal.ReflectionHelper.GetNonNullableType(memberType))
    ...
    //                            new ColumnExpression(NLite.Data.Linq.Internal.ReflectionHelper.GetMemberType(mex.Member), colType, alias, ""),
    //                            aggregator
    //                            );
    //                    }
    //                }
    ...
    //        if (NLite.Reflection.TypeHelper.IsGenericDictionaryType(instanceType))
    //        {
    ...
    //        EntityAssignment[] readonlyMembers = assignments.Where(b => NLite.Data.Linq.Internal.ReflectionHelper.IsReadOnly(b.Member)).ToArray();
    ...
    //                {
    //                    args[i] = Expression.Constant(NLite.Data.Linq.Internal.ReflectionHelper.GetDefault(p.ParameterType), p.ParameterType);
    //                    mis[i] = mems[0];
    //                }
    //                else
    //                {
    ...
    //            }
    //            else if(NLite.Reflection.TypeHelper.IsGenericDictionaryType(instanceType))
    //            {
    //                var dic = (instance as ConstantExpression).Value;
    //                var keys = (instanceType.GetProperty("Keys").GetGetter()(dic) as IEnumerable).Cast<string>();
    //                var getValueMethod = instanceType.GetMethod("get_Item").GetFunc();
